...
activation_criteria:
  keywords: [form state sync, save form data, submit to store, form zustand integration]
  file_patterns: ["**/*Form.tsx", "**/*Page.tsx"]
  modes: [form_builder, state_management]
provides:
  - Form-to-store data flow patterns
  - Submission handler integration with Zustand
...
### ❌ Passing raw set() to Components

```typescript
// WRONG: Passing raw set function
const set = useMyStore((state) => state.set);
<MyForm onSubmit={set} />

// CORRECT: Passing specific action
const updateData = useMyStore((state) => state.updateData);
<MyForm onSubmit={updateData} />
```
